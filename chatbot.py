...
def check_agreement(response, current_offer):
    agreement_prompt = f"""
    Analyze the following response in a negotiation context and determine if an agreement has been reached.
    Also, extract the final agreed price if an agreement is present.
    ...
    agreement_analysis = llm.invoke(agreement_prompt)
    
    try:
        import json
        result = json.loads(agreement_analysis.content)
        return result
    except json.JSONDecodeError:
        return {"agreement_reached": False, "final_price": None, "explanation": "Failed to parse LLM response"}
...
def chat(input_text, language, transport_cost, origin, destination, starting_price, max_price):
    ...
    agreement_result = check_agreement(content, transport_cost)
    return content, agreement_result
